...
func (c *Client) GetRegionAvailabilityZones(ctx context.Context, region string) ([]string, error) {
	...
	for _, rt := range provider.ResourceTypes {
		if rt.ResourceType != nil && *rt.ResourceType == "natGateways" {
			// Look for zone mappings for the specified region
			if rt.ZoneMappings != nil {
				for _, zm := range rt.ZoneMappings {
					if zm.Location != nil && strings.EqualFold(strings.ReplaceAll(strings.ToLower(*zm.Location), " ", ""), region) {
						if len(zm.Zones) > 0 {
							zones := make([]string, 0, len(zm.Zones))
			...
			// Region found in locations but no zone mappings = NAT gateway available but not zonal
			if rt.Locations != nil {
