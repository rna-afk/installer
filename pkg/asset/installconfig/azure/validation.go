...
func validateMininumRequirements(fieldPath *field.Path, req resourceRequirements, instanceType string, capabilities map[string]string) field.ErrorList {
	...
	if ok {
		...
		stdlogger.Warnf("could not find vCPUsAvailable information for instance type %s", instanceType)
	}
	...
	if ok {
		...
		stdlogger.Warnf("could not find MemoryGB information for instance type %s", instanceType)
	}
