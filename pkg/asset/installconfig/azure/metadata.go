...
func NewMetadataWithCredentials(cloudName typesazure.CloudEnvironment, armEndpoint string, credentials *client.Credentials) *Metadata {
	return &Metadata{
		CloudName:   cloudName,
...
func (m *Metadata) Session() (*client.Session, error) {
	m.mutex.Lock()
...
func (m *Metadata) unlockedSession() (*client.Session, error) {
	if m.session == nil {
		var err error
		m.session, err = client.GetSessionWithCredentials(m.CloudName, m.ARMEndpoint, m.Credentials)
		if err != nil {
...
func (m *Metadata) Client() (*client.Client, error) {
	if m.client == nil {
		ssn, err := m.Session()
		if err != nil {
			return nil, err
		}
		m.client = client.NewClient(ssn)
	}
	return m.client, nil
}
