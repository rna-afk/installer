...
func GenerateMachines(clusterID, resourceGroup, subscriptionID string, in *MachineInput) ([]*asset.RuntimeFile, error) {
	...
	var defaultDiag *capz.Diagnostics
	if in.Platform.DefaultMachinePlatform.BootDiagnostics != nil {
		defaultDiag = &capz.Diagnostics{
			Boot: &capz.BootDiagnostics{
				StorageAccountType: in.Platform.DefaultMachinePlatform.BootDiagnostics.Type,
			},
		}
		defaultURI := in.StorageAccountURI
		if in.Platform.DefaultMachinePlatform.BootDiagnostics.StorageAccountURI != "" {
			defaultURI = in.Platform.DefaultMachinePlatform.BootDiagnostics.StorageAccountURI
		}

		defaultDiag.Boot.UserManaged = &capz.UserManagedBootDiagnostics{
			StorageAccountURI: defaultURI,
		}
	}
