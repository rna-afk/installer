...
func MachineSets(clusterID string, config *types.InstallConfig, pool *types.MachinePool, osImage, role, userDataSecret string, capabilities map[string]string, session *icazure.Session) ([]*clusterapi.MachineSet, error) {
	...
	for idx, az := range azs {
		...
		provider, err := provider(platform, mpool, osImage, userDataSecret, clusterID, role, &idx, capabilities, session)
		if err != nil {
