...
func updateInternalLoadBalancer(ctx context.Context, in *lbInput) (*armnetwork.LoadBalancer, error) {
	...
	// For dual-stack, add IPv6 backend pool and MCS load balancing rule
	if in.isDualstack {
		...
		existingFrontEndIPConfigName := *(intLB.Properties.FrontendIPConfigurations[0].Name)
		mcsRuleV6 := &armnetwork.LoadBalancingRule{
			...
			Properties: &armnetwork.LoadBalancingRulePropertiesFormat{
				...
				LoadDistribution:     to.Ptr(armnetwork.LoadDistributionDefault),
				FrontendIPConfiguration: &armnetwork.SubResource{
					ID: to.Ptr(fmt.Sprintf("/%s/%s/frontendIPConfigurations/%s-v6", in.idPrefix, in.loadBalancerName, existingFrontEndIPConfigName)),
				},
				BackendAddressPool: &armnetwork.SubResource{
