...
func createDNSEntries(ctx context.Context, in clusterapi.InfraReadyInput, extLBFQDN, publicIP, intIPv6IP, resourceGroup string, opts *arm.ClientOptions) error {
	...
	ttl := int64(300)
	privateRecords = append(privateRecords, createPrivateRecordSet("api-int", azureTags, ttl, arecord, ipIlb, ""))
	privateRecords = append(privateRecords, createPrivateRecordSet("api", azureTags, ttl, arecord, ipIlb, ""))
	...
	if in.InstallConfig.Config.PublicAPI() {
		...
		if in.InstallConfig.Config.Azure.IPFamily.DualStackEnabled() {
			...
			publicRecordsV6 := createRecordSet(apiExternalNameV6, azureTags, ttl, cname, "", extLBFQDN)
			_, err = recordSetClient.CreateOrUpdate(ctx, baseDomainResourceGroup, zone, publicRecordsV6.Name, publicRecordsV6.RecordType, publicRecordsV6.RecordSet, nil)
			if err != nil {
				return fmt.Errorf("failed to create public record set: %w", err)
			}
		}
