...
func createDNSEntries(ctx context.Context, in clusterapi.InfraReadyInput, extLBFQDN, publicIP, intIPv6IP, resourceGroup string, opts *arm.ClientOptions) error {
	...
	if in.InstallConfig.Config.Azure.IPFamily.DualStackEnabled() {
		...
		if err == nil && ipIlbV6 != "" {
			privateRecords = append(privateRecords, createPrivateRecordSet("v6-api-int", azureTags, ttl, aaaarecord, ipIlbV6, ""))
			privateRecords = append(privateRecords, createPrivateRecordSet("v6-api", azureTags, ttl, aaaarecord, ipIlbV6, ""))
		}
	}
	...
	if in.InstallConfig.Config.PublicAPI() {
		...
		if in.InstallConfig.Config.Azure.IPFamily.DualStackEnabled() {
			apiExternalNameV6 := fmt.Sprintf("v6-api.%s", in.InstallConfig.Config.ObjectMeta.Name)
